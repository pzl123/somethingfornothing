                                               ...
                                               std::chrono::milliseconds ping_interval)
{
    ...
    d_log("LWS version: %s", lws_get_library_version());
    ...
    else
    {
        ...
        if (m_url.isValid() && (("ws" == m_url.getProtocol()) || ("wss" == m_url.getProtocol())))
        {
            ...
            // d_log("info.timeout_secs:%u, info.connect_timeout_secs:%u", info.timeout_secs, info.connect_timeout_secs);
...
int ocpp1_6::client::WebSocketClient::eventcb(struct lws *wsi, enum lws_callback_reasons reason, void *user, void *in, size_t len) noexcept
{
    ...
    if (nullptr != user)
    {
        self_client = reinterpret_cast<WebSocketClient *>(user);
    }
    // 处理不同的websocket事件
    switch (reason)
    {
    ...
    case LWS_CALLBACK_CLIENT_CONNECTION_ERROR:
    {
        ...
        const char *err = static_cast<const char *>(in);
        e_log("WebSocket connection error: %s", err ? err : "(null)");
        if (nullptr != self_client)
        {
            ...
            {
                e_log("No valid context for retry!");
            }
        }
        break;
    }
    case LWS_CALLBACK_CLIENT_APPEND_HANDSHAKE_HEADER:
    {
        ...
        if (nullptr != self_client)
        {
            ...
            {
                e_log("lws_http_basic_auth_gen2_ocpp error");
                break;
            }
            ...
            {
                e_log("lws_add_http_header_by_token error");
            }
        }
        break;
    }
